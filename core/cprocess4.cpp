...
int CProcess4::InitBeginOfTest(int nKind)
{
...
int CProcess4::GetProcessType()
{
    return 4;
}

bool CProcess4::ReadADData(int iCmdType,int iReadCout,int iTimeout)
{
    if(iTimeout < 1)
        iTimeout = 1;
...
bool CProcess4::ReadData(int iCmdType,int iSpaceType,int iReadCou)
{
    unsigned char bBuff[MAX_SIZE];
    memset(bBuff,0,MAX_SIZE);
    int iCount = 0;
...
void CProcess4::SaveSrcDataToTxt(int nKind)
{
...
//计算六价铬读取方式
double CProcess4::CaclWaySum2(int nKind)
{
...
//保存数据
int CProcess4::SaveDataToTx(int nKind)
{
    int iFlag = MCU_BUSY;
    int iDataLen = 0;
    if(m_nCalcWay == 1)
    ...
    {
        double dAbsorbency = log10(dBaseSignalVal / dSampleSignalVal);
        if(m_pOnBsReadValue)
        {
            m_pOnBsReadValue(m_userdata,12,dAbsorbency,m_Qtime);
        }
    }
...
//计算结果值
bool CProcess4::CalcTestValue(int nKey)
{
    char czValue[64];
...
void CProcess4::SaveResultToDatabase(int nCodeID)
{
...
int CProcess4::GetSysModuleStatus()
{
    int bRet = MCU_BUSY;
    switch(m_pro.PID)
    {
